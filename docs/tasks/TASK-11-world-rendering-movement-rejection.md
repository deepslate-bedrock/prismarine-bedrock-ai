...
- New follow-up evidence from a manual `examples/basic-bot.js` Endstone `1.26.12.2` run showed the bot could still send early `player_auth_input.position.y=1.62` before settling at the flat-world floor. That value means local feet `y=0` plus eye height, not a real server spawn height.
- Current patch now gates physics/auth-input sending until an authoritative movement position has arrived from a server movement packet (`respawn` state 1/2, `move_player`, or `correct_player_move_prediction`), and the sender refuses auth-input before that gate. It also preserves the authoritative feet position across the first auth-input so transient startup state cannot emit the old `1.62` packet.
- New follow-up on `2026-05-16`: public `bot.self.position` now stores the Bedrock eye position. Local physics converts that coordinate to feet only around simulation and support-block checks, while outgoing movement packets and interaction helpers use the public eye position directly. Eye/feet conversion is pose-aware: standing uses `Math.fround(1.62)`, sneaking uses `Math.fround(1.27)`, and low poses such as swimming/crawling/fall-flying use `Math.fround(0.4)`.
...
| `src/builtins/physics/index.js` | changed | Keeps authoritative movement packets as public eye coordinates, converts to feet only while running local physics, and stores a derived feet snapshot for compatibility/debugging. |
| `src/builtins/physics/movement-packets.js` | changed | Sends `player_auth_input`/`move_player` positions from public eye coordinates and aims `lookAt` from `self.position` directly; support-block checks derive feet first. |
| `src/builtins/physics/position.js` | added | Adds helpers for converting the public eye-position invariant to the feet position required by local physics/collision, deriving the offset from pose/pose-like flags with f32 Bedrock eye heights. |
| `src/builtins/dig.js`, `src/builtins/place.js`, `src/builtins/containers/index.js`, `src/builtins/food.js` | changed | Removed manual eye-height additions because `self.position` is now eye position. |
| `src/builtins/physics-constants.js` | changed | Stores the default standing eye-height constant with `Math.fround` precision. |
| `test/static/bedrock-rotation.test.js` | changed | Adds regression coverage that start/correction positions remain eye coordinates, auth-input sends that public coordinate unchanged, and pose-derived eye heights use f32 offsets. |
| `src/builtins/players.js` | changed | Avoids the generic player respawn mirror overwriting `botState.self`; self movement state is owned by the physics movement handlers. Stores standing/sneaking eye-height constants with `Math.fround` precision. |
...
- `2026-05-13` - Manual `examples/basic-bot.js` startup auth-input check before movement gate - FAIL. Command used Endstone `0.11.3` / BDS `1.26.12.2`, `MC_VERSION=1.26.10`, packet recorder split by player, and `node examples/basic-bot.js`. Decoded `logs/decoded-endstone-12612-lookatme-final-mybot-auth-position.jsonl` showed first auth-input at server floor `y=-59.37998962402344`, then a bad local reset packet at `y=1.6200000047683716` and a fall back to floor.
- `2026-05-13` - Manual `examples/basic-bot.js` startup auth-input check after movement gate - PASS. Command used Endstone `0.11.3` / BDS `1.26.12.2`, `MC_VERSION=1.26.10`, packet recorder split by player, and `node examples/basic-bot.js`. Decoded `logs/decoded-endstone-12612-lookatme-guarded-mybot-auth-position.jsonl` had one auth-input position sample, `y=-59.37998962402344`, with `hasNearOnePointSixTwo=false`. Syntax checks passed for `src/builtins/physics/index.js`, `src/builtins/physics/movement-packets.js`, and `src/builtins/players.js`.
- `2026-05-16` - Eye-position invariant static checks - PASS. Commands: `node -c src/builtins/physics/index.js`; `node -c src/builtins/physics/movement-packets.js`; `node -c src/builtins/physics/position.js`; `npx mocha test/static/bedrock-rotation.test.js`; `pnpm run test:static`. Notes: focused tests confirm physics start/correction handlers keep `self.position.y=65` for packet `y=65`, and `player_auth_input` sends public eye position `y=65.62` unchanged. Full static suite passed with 95 passing; only the existing Node `punycode` deprecation warning appeared.
- `2026-05-16` - Pose-aware eye-height static checks - PASS. Commands: `node -c src/builtins/physics/position.js`; `node -c src/builtins/physics-constants.js`; `node -c src/builtins/players.js`; `npx mocha test/static/bedrock-rotation.test.js`; `pnpm run test:static`. Notes: focused test now verifies standing `Math.fround(1.62)`, sneaking `Math.fround(1.27)`, and swimming `Math.fround(0.4)` offsets for eye-to-feet conversion. Full static suite passed with 96 passing; only the existing Node `punycode` deprecation warning appeared.
...
- `player_auth_input.position.y=1.62` during startup means local `self.position.y=0` plus eye height. It is not a valid server floor in the current Endstone superflat world; both the human client and BDS respawn sequence place players around auth-input `y=-59.38`.
- Initial movement ticks must not start from `set_spawn_position`, `start_game` placeholder heights, or high `respawn state=0` values. For current BDS `1.26.12.2`, the usable movement position arrives in the later respawn sequence around `y=-59.38`.
...
- Next step: rerun the focused TASK-06 Endstone crafting/workbench test that previously timed out waiting for `container_open`, using current `src/builtins/chunks.js` and movement startup gate.
