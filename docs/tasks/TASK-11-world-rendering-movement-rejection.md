...
| `src/builtins/physics/index.js` | changed | Gates movement ticks until an authoritative movement position arrives from respawn/move/correction packets, stores the authoritative feet position, and prevents `set_spawn_position` from starting auth-input ticks from placeholder startup coordinates. |
| `src/builtins/physics/movement-packets.js` | changed | Adds a packet-boundary guard so `player_auth_input` is not sent before the authoritative movement gate; the first auth-input after the gate uses the stored server-derived feet position. |
| `src/builtins/players.js` | changed | Avoids the generic player respawn mirror overwriting `botState.self`; self movement state is owned by the physics movement handlers. |
